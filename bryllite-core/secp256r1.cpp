...
static EccPoint curve_G = Curve_G_32;
...
bool create_key_pair( byte private_key[ECC_BYTES], byte public_key[ECC_BYTES+1] )
{
	...
	ecc_native2bytes( private_key, l_private );
	ecc_native2bytes( public_key + 1, l_public.x );
	public_key[0] = 2 + ( l_public.y[0] & 0x01 );
	return true;
}
...
bool make_shared_secret( const byte _private[ECC_BYTES], const byte _public[ECC_BYTES+1], byte _secret[ECC_BYTES] )
{
	...
	ecc_point_decompress( &l_public, _public );
	ecc_bytes2native( l_private, _private );

	EccPoint l_product;
	EccPoint_mult( &l_product, &l_public, l_private, l_random );

	ecc_native2bytes( _secret, l_product.x );
...
{
	return do_sign( (const byte*)&_privateKey, (const byte*)&_hash, (byte*)&_signature );
}
...
{
	return do_verify( (const byte*)&_publicKey, (const byte*)&_hash, (const byte*)&_signature );
}
...
bool do_verify( const byte _public[ECC_BYTES+1], const byte _hash[ECC_BYTES], const byte _signature[ECC_BYTES*2] )
{
	...
	ecc_point_decompress( &l_public, _public );
	ecc_bytes2native( l_r, _signature );
	ecc_bytes2native( l_s, _signature + ECC_BYTES );
